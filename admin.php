    ...
    <div class="app-shell">
      <aside id="adminSidebar" class="sidebar" aria-label="Admin navigation">
        ...
        <a class="nav-btn <?= $section === 'sms' ? 'active' : '' ?>" href="/admin?section=sms"><i
            class="bi bi-chat-dots"></i> SMS Logs</a>
      </aside>
      <button id="sidebarBackdrop" class="sidebar-backdrop" type="button" tabindex="-1"
        aria-label="Close navigation"></button>
      <main class="workspace">
        <header class="topbar">
          <button id="mobileMenuBtn" class="mobile-menu-btn" type="button" aria-label="Open navigation"><i
              class="bi bi-list"></i></button>
          <div>
            <h2><?= e(ucwords(str_replace('_', ' ', $section))) ?></h2><span class="muted"><?= e($user['name']) ?>
              (<?= e($user['role']) ?>)</span>
          </div>
          <div class="topbar-actions">
            <a href="/" class="btn btn-light btn-sm" target="_blank" rel="noopener"><i class="bi bi-globe2"></i>
              Website</a>
            <form method="post"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden"
                name="action" value="logout"><button class="btn btn-outline-danger btn-sm"><i
                  class="bi bi-box-arrow-right"></i> Logout</button></form>
          </div>
        </header>
        <?php if ($flash): ?>
          <div class="alert alert-<?= e($flash['type']) ?> admin-alert"><?= e($flash['message']) ?></div><?php endif; ?>
