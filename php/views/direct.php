...
<section class="work-section active">
  ...
  <form class="data-form" method="post" enctype="multipart/form-data">
    ...
    <div class="story-editor-group"><h4>Primary Contact</h4><div class="row g-3">
      <div class="col-md-4"><label class="form-label">Patient Full Name</label><input name="patient_name" class="form-control" required></div>
      <div class="col-md-4"><label class="form-label">Guardian Contact No.</label><input name="guardian_phone" type="tel" class="form-control" placeholder="+8801XXXXXXXXX" required></div>
      <div class="col-md-4"><label class="form-label">Email Address</label><input name="guardian_email" type="email" class="form-control"></div>
    </div></div>
    <?php foreach ($sections as $heading => $fields): ?>
      <div class="story-editor-group mt-4"><h4><?= e($heading) ?></h4><div class="row g-3">
        <?php foreach ($fields as $field): [$key,$label,$kind] = $field; $options = $field[3] ?? []; ?>
          ...
          <?php if ($kind === 'select'): ?><select name="payload_fields[<?= e($key) ?>]" class="form-select"><option value="">Select</option><?php foreach ($options as $option): ?><option value="<?= e($option) ?>"><?= e($option) ?></option><?php endforeach; ?></select>
          <?php elseif ($kind === 'textarea'): ?><textarea name="payload_fields[<?= e($key) ?>]" class="form-control" rows="2"></textarea>
          <?php else: ?><input name="payload_fields[<?= e($key) ?>]" type="<?= e($kind) ?>" class="form-control" <?= $key === 'diagnosis_type' ? 'required' : '' ?>><?php endif; ?></div>
        <?php endforeach; ?>
      </div></div>
    <?php endforeach; ?>
    <div class="story-editor-group mt-4"><h4>Attachments, Review, and Consent</h4><div class="row g-3">
      ...
      <div class="col-md-4"><label class="form-label">Registration Status</label><select name="status" class="form-select"><option>pending</option><option>accepted</option><option>rejected</option></select></div>
      <div class="col-md-8"><label class="form-label">Admin Notes</label><input name="notes" class="form-control"></div>
      <div class="col-12"><label class="form-check"><input type="checkbox" class="form-check-input" required> Consent confirmed for storing and using this registration information.</label></div>
    </div></div>
