...
function handle_content_action(string $action, array $user): never
{
    ...
    if ($action === 'content_toggle_publish') {
        ...
        flash($newStatus === 1 ? 'Content published.' : 'Content unpublished.');
    ...
    if ($action === 'content_delete') {
        ...
        if (!in_array($type, ['gallery', 'patient_story'], true)) {
        ...
        redirect_admin('content', ['type' => 'gallery', 'media' => $mediaType]);
    }
    ...
    if ($id) {
        ...
        flash($type === 'patient_story' ? 'Patient story updated.' : 'Content updated.');
    } else {
        $values[] = $user['id'];
